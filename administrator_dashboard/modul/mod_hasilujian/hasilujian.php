...
// Tombol cetak data
          ...
          onclick=\"window.location.href='modul/mod_hasilujian/cetak.php';\">";

 //Form Pencarian Data
                          echo "<div style='text-align:left;padding-right:40px;max-width:360px;'>
         <form method='POST' action=?module=hasilujian&act=cariuser>
     <input type=text name='cari'  placeholder='Masukkan nama peserta' list='auto' size=26 required/> <input type=submit class='btn btn-info' value='Cari'>";
                          ...
                          $qry = mysqli_query($mysqli, "SELECT * FROM tbl_nilai,tbl_user WHERE tbl_nilai.id_user=tbl_user.id_user ORDER BY tbl_nilai.benar DESC");
...
<?php          
 ...
 ?>  
   ...
   <td align ='center'><a href="modul/mod_hasilujian/aksi_hasilujian.php?module=hasilujian&id=<?=$r['id_nilai']?>&iduser=<?=$r['id_user']?>" class="btn btn-danger btn-xs delete-data">Hapus</a></td>
   </tr>
...
<?php   
                          ...
                          $tampil = mysqli_query($mysqli, "SELECT * FROM tbl_nilai,tbl_user WHERE  nama LIKE '%$_POST[cari]%' and tbl_nilai.id_user=tbl_user.id_user ORDER BY tbl_nilai.benar DESC");
  $no = 1;
                        while ($r = mysqli_fetch_array($tampil)) {
                          $tgl = tgl_indo($r['tanggal']);
 ?>  
...
if(notifikasi == "Disimpan" || notifikasi=="Dihapus"){
                      ...
                      setTimeout(function() {     
                      window.location='../../ujianonline/admin/media.php?module=hasilujian';
                                              }, 1500); 
