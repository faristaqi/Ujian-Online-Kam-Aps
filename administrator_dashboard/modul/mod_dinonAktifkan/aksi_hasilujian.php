...
// Update user
if ($module == 'mod_dinonAktifkan' and $act == 'update') {
...
} elseif ($module == 'mod_dinonAktifkan' and $act == 'hapus') {
...
} elseif ($module == 'mod_dinonAktifkan' and $act == 'input') {
  $lokasi_file = $_FILES['fupload']['tmp_name'];
  $nama_file   = $_FILES['fupload']['name'];
...
//Pengaktifan dan Pengnonaktifan pengalihan halaman web ke modul = mod_dinonAktifkan
elseif ($module == 'mod_dinonAktifkan' and $act == 'nonaktif') {
  $aktif = 'N';
  mysqli_query($mysqli, "UPDATE tbl_user SET statusaktif  = '$aktif'  WHERE id_user='$_GET[id]'");
  header('location:../../media.php?module=' .  $module);
} elseif ($module == 'mod_dinonAktifkan' and $act == 'aktif') {
  $aktif = 'Y';
  mysqli_query($mysqli, "UPDATE tbl_user SET statusaktif  = '$aktif'  WHERE id_user='$_GET[id]'");
  header('location:../../media.php?module=' .  $module);
}
