...
<?php
                    $aksi = "modul/mod_dinonAktifkan/aksi_hasilujian.php";
                    switch ($_GET['act']) {
                        // Tampil User
                      default:
                        $tampil = mysqli_query($mysqli, "SELECT * FROM tbl_user WHERE statusaktif='N' ORDER BY nama ASC");
 ?>  
          <h4> Daftar Peserta Yang di Non Aktifkan</h4>
...
<?php 
 ...
 //Form Pencarian Data
         ...
         <form method='POST' action=?module=mod_dinonAktifkan&act=cariuser>
...
<form  method="POST" name="form1" action="modul/mod_dinonAktifkan/delete_all.php" >
...
echo " 
  ...
  <td align=center ><a class='fa fa-pencil-square-o' style='font-size:20px;' onclick=\"window.location.href='modul/mod_dinonAktifkan/profileuser.php?module=mod_dinonAktifkan&id=$r[id_user]';\"></a></td>
...
";        
                           ...
                           if ($r['statusaktif'] == "Y") { 
                            echo "<td align=center><input type=button class='btn btn-success btn-xs'  value='Aktif' onclick=\"window.location.href='$aksi?module=mod_dinonAktifkan&act=nonaktif&id=$r[id_user]';\"></td>";
                          } else {
                            echo "<td align='center'><input class='btn btn-default btn-xs' type=button value='Non Aktif' onclick=\"window.location.href='$aksi?module=mod_dinonAktifkan&act=aktif&id=$r[id_user]';\"></td>";
                          }
 ...
 <!--  class : delete data untuk mengkoneksikan dengan sweet alert href mengirimkan link dan data id (PENTING !!! BIAR KONEK DENGAN SWEET ALERT HARUS <TD> </TD> DALAM BENTUK HTML MAKA PENULISANNYA JUGA BEDA DENGAN YG DI PHP perhatikan petik nya dll ) ) -->        
             <td align=center><a class='btn btn-danger btn-xs delete-data' href="modul/mod_dinonAktifkan/aksi_hasilujian.php?module=mod_dinonAktifkan&act=hapus&id=<?=$r['id_user']?>">Hapus</a></td>
...
<form  method="POST" name="form1" action="modul/mod_dinonAktifkan/delete_all.php" >
   ...
   <table class='table table-striped table-bordered table-hover'>
          <tr><th style='text-align: center'>No</th><th style='text-align: center'>User name</th><th style='text-align: center'>Nama</th><th style='text-align: center'>Password</th><th style='text-align: center'>Jenis Kelamin</th><th style='text-align: center'>Edit</th><th style='text-align: center'>Lihat</th><th style='text-align: center'>Status</th><th width="10"><input type="checkbox" id="check-all"></th><th width="100" style="text-align: center"> <button id="btn-delete" class="btn btn-danger mb-4 mt-4 hapus_all" >Delete Selected</button></th></tr>
<?php
                          $tampil = mysqli_query($mysqli, "SELECT * FROM tbl_user WHERE  nama LIKE '%$_POST[cari]%' and  statusaktif='N' ORDER BY nama ASC");                       
                       $no = 1;
                        while ($r = mysqli_fetch_array($tampil)) {
 ?>                           
            <tr><td><?php echo $no ; ?></td>
...
if(notifikasi == "Disimpan" || notifikasi=="Dihapus"){
                      ...
                      setTimeout(function() {     
                      window.location='../../ujianonline/admin/media.php?module=mod_dinonAktifkan';
                                              }, 1500); 
