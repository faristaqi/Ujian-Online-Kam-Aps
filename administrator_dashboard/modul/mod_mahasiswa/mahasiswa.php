...
switch ($_GET['act']) {
  ...
  default:
    ...
    $qry = mysqli_query($mysqli, "SELECT * FROM tbl_mahasiswa");
    ...
    $tampil = mysqli_query($mysqli, "SELECT * FROM tbl_mahasiswa");
    $no = 1;
  ...
  case "editmahasiswa":
    $sql = mysqli_query($mysqli, "SELECT * FROM tbl_mahasiswa WHERE id='$_GET[id]'");
  ...
  case "carimahasiswa":
    ...
    $tampil = mysqli_query($mysqli, "SELECT * FROM tbl_mahasiswa WHERE id LIKE '%$_POST[cari]%' or nama LIKE '%$_POST[cari]%'");
    $no = 1;
    while ($r = mysqli_fetch_array($tampil)) {
      echo "
	   		 <tr><td>$no</td>
