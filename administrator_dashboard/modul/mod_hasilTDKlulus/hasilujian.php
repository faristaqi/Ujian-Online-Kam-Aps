...
// Seleksi seluruh data dan gabungkan dari tabel tbl_nilai dgn tbl_user (yang gabungan tsb tampilkan berdasar id_user yang ada di tabel tbl_user) {dan dgn syarat tbl_nilai = keterangan[tidak lulus]} kemudian tampilkan berdasar nilai tertinggi (ASC)                      
                        $tampil = mysqli_query($mysqli, "SELECT * FROM tbl_nilai,tbl_user WHERE tbl_nilai.id_user=tbl_user.id_user and tbl_nilai.keterangan='Tidak Lulus' ORDER BY tbl_nilai.benar DESC ");
 // Tombol cetak data
                          echo "<div style='text-align:left;padding-left:2px;'>
          <input class='btn btn-secondary' type=button value='Cetak Data' 
          onclick=\"window.location.href='modul/mod_hasilTDKlulus/cetak.php';\">";

 //Form Pencarian Data
                          echo "<div style='text-align:left;padding-right:40px;max-width:360px;'>
         <form method='POST' action=?module=mod_hasilTDKlulus&act=cariuser>
 ...
 ?> 
        <h4>Peserta Yang Tidak Lulus Berdasarkan Score</h4>
...
<?php
 ...
 ?> 
   ...
   <td align ='center'><a href="modul/mod_hasilTDKlulus/aksi_hasilujian.php?module=mod_hasilTDKlulus&id=<?=$r['id_nilai']?>&iduser=<?=$r['id_user']?>" class="btn btn-danger btn-xs delete-data">Hapus</a></td>
   </tr>
<?php  
                          $no++;
                        }
...
<?php
                          ...
                          $tampil = mysqli_query($mysqli, "SELECT * FROM tbl_nilai,tbl_user WHERE  nama LIKE '%$_POST[cari]%' and tbl_nilai.id_user=tbl_user.id_user and tbl_nilai.keterangan='Tidak Lulus' ORDER BY tbl_nilai.benar DESC");
                          $no = 1;
                          while ($r = mysqli_fetch_array($tampil)) {
                          $tgl = tgl_indo($r['tanggal']); 
 ?>  
...
if(notifikasi == "Disimpan" || notifikasi=="Dihapus"){
                      ...
                      setTimeout(function() {     
                      window.location='../../ujianonline/admin/media.php?module=mod_hasilTDKlulus';
                                              }, 1500); 
